...
La interfaz entre funciones de *assembly* y un código de programa en C vienen definidas en el documento AAPCS (ARM Architecture Procedure Call Standard).  
Cuando un código de programa en C llama a una función en *assembly*, los argumentos escritos en C se reciben en los registros R0, R1, R2 y R3. De recibir más de 4 parámetros, estos comienzan a colocarse en la pila (*stacking*). Una función con más de 4 parámetros deberá hacer POP (*unstacking*) para colocar dichos parámetros en los registros R4 a R11.  
El resultado se devuelve dependiendo del tipo de dato a retornar, si la precisión del tipo de datos es menor o igual a 32 bits, el resultado se devuelve por R0. Si el tipo de dato es de 64 bits, el resultado se devuelve por R0 y R1. Si el tipo de dato es de 128 bits, el resultado se devuelve por R0, R1, R2 y R3.  
Si se realizan operaciones con los registros R4 a R11, el contenido de estos debe colocarse en la pila con PUSH (*stacking*), para luego al final de la ejecución de la función restaurar dichos registros (R4 a R11) con su contenido inicial, mediante POP (*unstacking*).
## 5. ¿Qué es una instrucción SIMD? ¿En qué se aplican y que ventajas reporta su uso? Dé un ejemplo.
Una instrucción SIMD (Single Instruction Multiple Data) es un tipo de instrucción que permite, como su nombre lo indica, llevar a cabo múltiples operaciones de datos en paralelo.  
Se aplican en procesamiento digital, como por ejemplo procesamiento de audio (donde el canal L y R pueden procesarse al mismo tiempo) o procesamiento de imágenes (donde los bits RGB pueden procesarse en paralelo).  
La ventaja de su uso radica en que el procesamiento en paralelo evita la distorsión de los resultados de este tipo de aplicaciones. Por ejemplo, si se procesan los bits RGB de un píxel en tiempos distintos, el color aparante del píxel será diferente del que esperado. Varias instrucciones de aritmética saturada incluyen versiones que soportan SIMD, ya que en procesamiento digital es de interés que los cálculos no se distorsionen de manera extrema (como implica un desbordamiento). La distorsión aceptable, justamente, es la saturación de la señal.
